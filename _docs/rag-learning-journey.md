...
**做什么**：node_pdf_to_md 通过 MineRU 异步 API（提交→轮询→下载）将 PDF 转为 Markdown；node_md_img 提取图片占位符，调用 VLM 描述图片内容，回填描述文字到 MD 正文。

**不做什么**：不负责切分和向量化；MineRU 异常直接 raise 终止工作流，不产出半成品。

**关键取舍**：图片描述回填到正文而非单独存储，确保切分后图片语义随文字一起被向量化，检索时可命中图片内容。
