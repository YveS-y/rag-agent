...
**做什么**：rag-agent 分导入图和查询图两个 LangGraph 工作流。导入图接收 PDF/MD 文件，经 PDF 解析→图片处理→切分→主题识别→向量化→入库 Milvus；查询图先确认查询范围，再并行 3 路召回（向量检索/HyDE/Web），RRF 融合排序后生成回答。

**不做什么**：非 PDF/MD 格式静默终止不报错；不训练或微调模型。
...
**做什么**：node_pdf_to_md 通过 MineRU 异步 API（提交→轮询→下载）将 PDF 转为 Markdown；node_md_img 提取图片占位符，连同图片前后100字上下文一起调用 VLM 生成语义描述，再上传图片到 MinIO（按文档名分文件夹），把描述+实际地址回填到 md 正文，存为新文件。

**不做什么**：不负责切分和向量化；不修改原始 md 文件内容；MineRU 异常直接 raise 终止工作流，不产出半成品。

**关键取舍**：图片描述回填到正文而非单独存储，确保切分后图片语义随文字一起被向量化；上传前先按文档名清空 MinIO 旧对象再重传，保证同一文档重复导入时的幂等性，避免陈旧图片越积越多。

### L3 · node_document_split

**做什么**：把文档内容切分成 chunk，供后续向量化和召回使用。

**不做什么**：不做向量化处理；不连接其他数据库或大模型。
