...
**更新** 基于测试.txt中的RAG项目架构说明，新增了节点分类标准（"做什么"、"不做什么"、"关键取舍"）和完整的导入、查询阶段流程指导，为单元测试提供更清晰的架构指导。
...
**更新** 基于测试.txt的架构说明，导入阶段负责处理.pdf和.md文件，进行主题分词、图片内容转换、存储到MinIO服务器、文本内容和图片说明存到Milvus、历史会话记录存到MongoDB、知识图谱存到Neo4j数据库。查询阶段负责多路召回（向量化、HyDE假设性、WEB）并通过算法计算排名后得到最终结果。
...
- [app/query_process/agent/state.py:1-97](file://app/query_process/agent/state.py#L1-L97)
- [app/core/logger.py:1-95](file://app/core/logger.py#L1-L95)
- [测试.txt:16-18](file://测试.txt#L16-L18)
...
**更新** 基于测试.txt的节点分类标准，导入流程的节点职责明确：
- "做什么"：处理PDF/MD文件转换、文本切分、向量化、Milvus入库
- "不做什么"：不负责查询检索、不处理其他格式文件
- "关键取舍"：选择PDF转MD再处理的架构，平衡准确性与性能
...
**更新** 基于测试.txt的节点分类标准，查询流程的节点职责明确：
- "做什么"：多路召回（向量化、HyDE、WEB）、RRF融合、重排序、答案生成
- "不做什么"：不负责文档导入、不处理文件存储
- "关键取舍"：采用多路召回+RRF融合的架构，提升召回质量
...
**更新** 基于测试.txt的架构说明，状态管理需要考虑：
- 导入阶段：任务ID、流程控制标记、路径信息、内容数据、数据库相关字段
- 查询阶段：会话ID、原始问题、检索中间结果、排序结果、生成结果、辅助信息
...
**更新** 基于测试.txt的节点分类标准，节点执行测试应重点关注：
- 导入节点分类：
  - "做什么"：PDF/MD文件处理、文本切分、向量化、Milvus入库
  - "不做什么"：不处理其他格式、不负责查询
  - "关键取舍"：选择特定的处理流程以平衡性能与准确性
- 查询节点分类：
  - "做什么"：多路召回、融合排序、答案生成
  - "不做什么"：不负责文档导入、不处理文件存储
  - "关键取舍"：采用多路召回策略提升召回质量
...
**更新** 基于测试.txt的故障排查指导：
- Kafka权限问题：检查用户权限配置，确保只具备必要的读写权限
- Kerberos认证：验证keytab文件和JAAS配置，检查krb5.conf设置
- 文件格式：确保只处理PDF和MD格式，其他格式应报错
- 外部服务：监控Milvus、MinIO、MongoDB、Neo4j的连接状态
...
## 结论
本文件提供了导入主流程与查询主流程的单元测试实现与设计指导，强调了状态管理、节点执行与错误处理的测试策略。通过合理的测试数据准备、断言与日志配置，可以有效保障LangGraph工作流的稳定性与可靠性。

**更新** 基于测试.txt的架构说明，建议在新增节点或修改流程时同步更新测试用例，并持续优化性能与可观测性。同时，应重点关注节点分类标准的实施，确保每个节点都明确自己的职责边界和关键取舍。
...
- 编写新测试用例与维护既有测试
  ...
  - 节点分类标准应用：
    - "做什么"：明确节点的核心职责和功能边界
    - "不做什么"：定义节点的职责边界，避免功能蔓延
    - "关键取舍"：说明设计决策的原因和权衡考虑

**更新** 基于测试.txt的节点分类标准，新测试用例应明确标注每个节点的职责分类，确保测试覆盖全面且有针对性。
